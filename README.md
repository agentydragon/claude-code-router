...
Claude Code Router includes request/response tracing for debugging and monitoring. When enabled, it logs four key events in the request lifecycle with correlation IDs:
...
> **Note**: The UI mode is currently in beta. If you encounter any issues, please submit an issue on GitHub.
...
- **openai-reasoning**: Applies compatibility fixes for OpenAI reasoning models (configure via TransformerOptions).
...
- `openai-reasoning`: Configure via TransformerOptions with patterns for your target models.
- `system-replace`: Configure via TransformerOptions for search/replace of system messages.
